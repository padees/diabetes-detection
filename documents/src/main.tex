...
% Diabetes in numbers
In 2021, there were 529 million people living with diabetes worldwide, and the global age-standardised total diabetes prevalence was 6·1% \cite{ong2023global}.
A similar conclusion was reached by Sun et. al \cite{sun2022idf}.


It is estimated that 537 million people in the age range of 20 to 79 are affected by diabetes currently, and this number will grow to 643 million by 2045.
...
There are increasing ethical concerns about the unequal impact these models may have on underrepresented communities \cite{chen2023algorithmic}.
...
Addressing biases in medical AI requires continued research, larger and more representative patient data, and improved transparency in literature. Efforts to detect and mitigate bias, particularly in clinical decision-making, are essential to prevent health disparities and ensure equitable benefits for all patients \cite{cross2024bias}.
...
This limitation stems from the lack of datasets that include sufficient data points for type-specific classification. To bridge this gap, this study proposes a first stage and second stage :
...
Many attempts have been made to successfully integrate Machine Learning techniques for detection of diabetes, and several different techniques have been used for the same\cite{sharma2021comprehensive}.
...
It has been shown that ensemble techniques perform better than logistic regression, due to the fact that Logistic Regression sufferrs from assuming linear relationships and is much more sensitive to outliers.
...
Rajendra and Latifi concluded that the ensemble techinques Max Voting and Stacking fare better than Logistic Regression, with the latter having provided evidence that Gradient Boosted Decision Trees (GBDT) and logistic regression models perform somewhat equally upto training data size of $10^{4}$, GBDT shows a significant increase after this number, while Logistic Regression the model's metrics become saturated\cite{rajendra2021prediction}.

Seto et. al provided evidence that Gradient Boosted Decison Trees (GBDT) and logistic regression models performsomewhat equally upto training data size of 10000. GBDT shows a significant increase after this number, while the logistic regression model's metrics become saturated \cite{seto2022gradient}.

Salem et al. utilised the basic K - nearest neighbours and two of its variants which allow partial membership in multiple classes, Fuzzy KNN and Three - Fold Nested K-Nearest Neighbours. TFKNN turned out to be the best performer with 94.13\% specific accuracy\cite{salem2022fine}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{../../assets/feature_importance/correlation_matrix.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{../../assets/matrices/decisionTree.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{../../assets/matrices/randomForest.png}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{../../assets/matrices/xgboost_train.png}
        \caption{XGBoost confusion matrix (Train)}
        \label{fig:bayes_matrix_train}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{../../assets/matrices/xgboost_test.png}
        \caption{XGBoost confusion matrix (Test)}
        \label{fig:bayes_matrix_test}
    \end{minipage}
\end{figure}
...
\subsection{Results}
In this section, we present the results of our machine learning experiments for diabetes detection using various supervised learning algorithms. We interpret the performance of each model based on relevant metrics such as accuracy, precision, recall, F1-score, specificity, Matthews Correlation Coefficient (MCC), and Area Under the Curve (AUC) \cite{kavakiotis2017machine, gautier2021artificial}. The findings are visualized and compared comprehensively to determine the most effective classifier for the problem domain.

Performance of the Decision Tree classifier was optimized using cost-complexity pruning with cross-validation. The optimal \texttt{ccp\_alpha} value was determined through a 10-fold Stratified K-Fold cross-validation process, balancing generalization and model simplicity. The final pruned tree achieved a testing accuracy of approximately 80\%, indicating a robust fit without overfitting the training data \cite{gupta2022comparing}. The classification report demonstrated fairly balanced precision and recall values across the target classes, with the F1-score reflecting this balance.

From Figure 2, we can conclude that decision trees exhibit a worse misclassification ratio between type 2 and type 3c diabetes compared to other models \cite{ordonez2023machine, zhong2020classification}. This indicates that the tree model struggles more with separating these two clinically similar classes. While classification of prediabetes and type 1 remains accurate, type 2 instances are more frequently mislabeled as type 3c. The rigid splits of decision trees likely fail to capture subtle feature distinctions. This suggests limited generalization for overlapping diabetes subtypes.

Figure 3 shows that the Random Forest achieves perfect classification for prediabetes (0), type 1 diabetes (1), and type 3c diabetes (3) in both training and testing. Type 2 diabetes (2) is frequently misclassified as type 3c, with 411 training and 295 testing errors. This suggests significant feature overlap between type 2 and type 3c diabetes \cite{mani2012type, febrian2023diabetes}. Class separation is robust for prediabetes and type 1, but weak between type 2 and 3c.

This is in line with findings by Phongying and Hiriote, who found that Random Forest technique is superior to Support Vector Machines, Decision Trees, and K-nearest neighbour methods \cite{phongying2023diabetes}.

From Figure 4 and 5, it is clear that XGBoost achieves the most consistent and reliable classification across all four diabetes subtypes, showing no errors in identifying prediabetes and type 1 diabetes, and substantially lower confusion between type 2 and type 3c compared to decision trees \cite{lacoste2012bayesian}. This superior performance can be attributed to XGBoost’s gradient boosting framework, which combines multiple weak learners into a strong ensemble, allowing it to capture complex, nonlinear feature interactions more effectively. Unlike standard decision trees, XGBoost incorporates regularization to prevent overfitting, handles missing data internally, and optimizes using second-order gradients, which enhances convergence and model generalization. These advantages are particularly beneficial in medical datasets where feature dependencies and subtle variations across classes are common. Consequently, XGBoost provides better class separation in high-dimensional clinical feature spaces, especially when dealing with overlapping phenotypes such as type 2 and type 3c diabetes.

Logistic Regression handles Prediabetes (0) and Type 1 (1) very well, nearly perfect in both train and test sets. Type 2 (2) and Type 3c (3) are frequently confused—898 training and 240 testing samples of Type 2 are misclassified as Type 3c, and 736 training and 189 testing samples of Type 3c are misclassified as Type 2. Accuracy drops to 89.98\% (train) and 89.48\% (test), worse than DB. Feature overlap between Type 2 and Type 3c severely degrades performance. Model is too simplistic for this distinction. Hence this model was not chosen.

The confusion matrices further revealed that the Decision Tree classifier was moderately effective in correctly classifying samples across all categories, although minor misclassifications between adjacent or overlapping conditions (e.g., class 2 vs. class 3) were observed. The feature importance plot provided insights into which attributes most influenced the model's decisions. In this case, features like \texttt{Blood Glucose Level}, \texttt{Insulin}, and \texttt{BMI} were identified as strong contributors to the model’s decisions, aligning with known medical knowledge regarding diabetes biomarkers \cite{zhong2020classification}.

In contrast, the Naive Bayes classifier, optimized using \texttt{GridSearchCV} over the \texttt{var\_smoothing} parameter, showed slightly lower performance compared to the Decision Tree. The best model achieved a test accuracy of approximately 78\%, with a relatively higher precision but slightly lower recall. This indicates that while the model was confident in its positive predictions, it had difficulty in identifying all relevant instances across classes \cite{sethi2019comparative}. Given that Naive Bayes assumes independence between features, the slightly diminished performance is expected, especially in a dataset where attributes like BMI and insulin levels may be correlated.

Nonetheless, the confusion matrix for Naive Bayes revealed that the classifier managed to achieve reasonably good separation across the five target classes, although misclassifications were more prominent when compared to the Decision Tree. Particularly, class overlaps and false positives in adjacent classes highlighted the limitation of the strong independence assumption of the model.

A consolidated table comparing all the models implemented (including additional models such as Logistic Regression, K-Nearest Neighbors, Random Forest, XGBoost, and Voting Classifier) demonstrates that ensemble-based approaches consistently outperformed individual classifiers \cite{gautier2021artificial, kavakiotis2017machine}. XGBoost achieved the highest testing accuracy of 88\%, followed closely by the Voting Classifier and Random Forest. These models also recorded the highest F1-scores and AUC values, signifying strong discrimination capabilities. Decision Tree and KNN offered a balance between interpretability and accuracy, while Naive Bayes and Logistic Regression, though slightly less accurate, maintained good precision.

\subsection{Conclusion}
This study investigated the effectiveness of several machine learning models for multi-class classification of diabetic conditions using a real-world dataset. Through comprehensive evaluation using stratified cross-validation and hyperparameter optimization, we identified that ensemble-based models---specifically XGBoost and the Voting Classifier---consistently achieved superior predictive performance \cite{ordonez2023machine, febrian2023diabetes}.

The Decision Tree classifier, when properly pruned and regulated, offered a balance between interpretability and accuracy, making it a viable choice for clinical settings where explainability is crucial \cite{gupta2022comparing}. Meanwhile, Naive Bayes, despite its simplifying assumptions, demonstrated respectable performance and could serve as a lightweight baseline model.

Our experiments affirm the critical role of appropriate preprocessing (such as feature scaling and encoding), hyperparameter tuning, and model selection in achieving optimal performance in health-related predictive analytics. Furthermore, feature importance analysis confirmed the influence of key medical indicators such as glucose, insulin, and BMI in predicting diabetes-related outcomes \cite{zhong2020classification}.

Future work can explore model calibration, deeper hyperparameter optimization (e.g., via Bayesian search), and integration of additional clinical features such as genetic markers or lifestyle variables to enhance predictive power. Moreover, deploying explainable AI techniques like SHAP or LIME could further aid in understanding and validating model decisions in real-world medical environments.
