...
Methods:
To address the limitations of existing models—particularly their inability to differentiate between diabetes types due to limited data—a compounded dataset was created by merging two publicly available sources. The model performs binary classification to detect diabetes presence and multiclass classification to identify type, including Prediabetes, Type 1, Type 2, and Pancreatogenic Diabetes. Algorithms implemented include Naive Bayes, K-Nearest Neighbours, Logistic Regression, Random Forest, and XGBoost.
...
\includegraphics[width=0.5\textwidth]{../../assets/feature_importance/correlation_matrix.png}
